...
export async function GET(request: Request) {
  ...
  let query = supabase.from('health_facilities')
    .select('*, locations(name, district_name, block_name)', { count: 'exact' })
    .order('name').range(from, from + limit - 1)
  if (q) query = query.or(`name.ilike.%${q}%,health_block.ilike.%${q}%,district.ilike.%${q}%,facility_type.ilike.%${q}%`)
